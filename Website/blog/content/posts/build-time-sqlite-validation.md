...
### Generating the manifest instead of typing it

You don't have to write those `parameters` and `results` blocks by hand. If you already build `XLStaticQueryDescriptor` values, `SQLiteBuildValidationQueryEntry` projects one straight into a manifest entry:
...
The projection derives the SQL, the parameter layout, and the result columns from the descriptor. It recovers each parameter's physical placeholder position by scanning the rendered SQL, so a query whose logical argument order differs from its `?N` positions still gets correct indices. Mismatches are caught while generating rather than at validation time: passing the wrong number of `declaredAliases` throws `resultAliasCountMismatch`.

`canonicalJSONData()` sorts keys and normalises the entries, so regenerating an unchanged manifest produces a byte-identical file and creates no diff noise.
...
Manifest generation reaches as far as `XLStaticQueryDescriptor` and stops there. Queries declared with `@SQLQuery` are built on the older `sql { }` statement path and deliberately do not produce a descriptor yet, so there is no automatic route from a declaration to a manifest entry. Until that lands you either construct descriptors yourself and project them, or write the entries by hand.
