...
function: one positional argument per stored property, in declaration order.
The `@SQLFunction` macro generates both members from those properties, so a
conformer only has to declare the arguments and implement `execute`. Attach the
macro and remove the two generated members from `HaversineDistance` above:
...
public struct HaversineDistance: XLCustomFunction {
    ...
    // Define parameters which are passed to the function at runtime. Each
    // stored property becomes one SQL argument, in declaration order.
    ...
    // `definition` and `makeSQL(context:)` are generated by `@SQLFunction`.
    // Only `execute` — the actual computation — is still written by hand.
    public static func execute(reader: XLColumnReader) throws -> Double {
        let latA = try radians(degrees: reader.readReal(at: 0))
        let lonA = try radians(degrees: reader.readReal(at: 1))
        let latB = try radians(degrees: reader.readReal(at: 2))
